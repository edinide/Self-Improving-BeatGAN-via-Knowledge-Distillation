...
class BeatGAN_t(AD_MODEL):


    def __init__(self, opt, dataloader, device):
        super(BeatGAN_t, self).__init__(opt, dataloader, device)
...
class BeatGAN_s(AD_MODEL):
    def __init__(self, opt, dataloader, device):
        ...
        self.teacher=BeatGAN_t(opt, dataloader, device) #teacher객체 생성
    ...
    def train_epoch(self):
        ...
        for data in self.dataloader["train"]:
            ...
            if (epoch_iter  % self.opt.print_freq) == 0: #iter이 prin_freq(=100)단위 될때마다 console출력
    ...
    def set_input(self, input):
        #[error] self.input.data.resize_(input[0].size()).copy_(input[0])
        with torch.no_grad():
            self.input.resize_(input[0].size()).copy_(input[0])
        #[error]self.gt.data.resize_(input[1].size()).copy_(input[1])
        with torch.no_grad():
            self.gt.resize_(input[1].size()).copy_(input[1])
