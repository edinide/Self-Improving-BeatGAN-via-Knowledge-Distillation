...
def getPercent(data_x,data_y,percent,seed):
    ...
    #데이터분할을 위한 함수 train_test_split으로, 전체 데이터셋 배열을 받아 랜덤하게 test/train 데이터 셋으로 분리해줌
    #test_size를 지정할 경우, 테스트 데이터셋을 뺀 나머지를 훈련 데이터로 사용
    #즉 data_x와 data_y에 대해 각각 (line78에 의하면 test_size=0.1이므로) 10퍼센트가 test_x, test_y에, 나머지가 train_x, train_y로 분리됨
